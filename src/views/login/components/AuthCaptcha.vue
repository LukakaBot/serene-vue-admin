...
function drawLine(ctx: CanvasRenderingContext2D) {
  for (let i = 0; i < 5; i++) {
    ctx.beginPath();
    ctx.moveTo(generateRandomNumber(0, props.width), generateRandomNumber(0, props.height));
    ctx.lineTo(generateRandomNumber(0, props.width), generateRandomNumber(0, props.height));
...
function drawPoint(ctx: CanvasRenderingContext2D) {
  for (let i = 0; i < 40; i++) {
    ctx.beginPath();
    ctx.arc(generateRandomNumber(0, props.width), generateRandomNumber(0, props.height), 1, 0, 2 * Math.PI);
...
function drawCaptcha() {
  ...
  if (props.drawLine) drawLine(ctx);
  if (props.drawPoint) drawPoint(ctx);
}
